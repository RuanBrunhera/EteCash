...
export default function CarrinhoVenda({ onClose }) {
  ...
  const produtosFiltrados = produtos.filter((produto) =>
    produto.nome.toLowerCase().includes(buscaProduto.toLocaleLowerCase()),
  );
  ...
  async function buscarAluno() {
    ...
    try {
      const { data, error } = await vendaService.buscarAlunoPorRm(rmInput);
  ...
  useEffect(() => {
    ...
    async function carregarProdutos() {
      ...
      try {
        const { data, error } = await vendaService.listarProdutos()

        if (error){
          setErroProdutos(error)
          return
        }
  ...
  function alterarQuantidade(produtoId, delta) {
    ...
    setDadosVenda((prevDadosVenda) => {
      if (novaQuantidade === 0) {
        ...
        return {
          ...prevDadosVenda,
          itens: prevDadosVenda.itens.filter(
      ...
      if (quantidadeAtual === 0) {
        ...
        return {
          ...prevDadosVenda,
          itens: [
            ...prevDadosVenda.itens,
            { produto_id: produtoId, quantidade: novaQuantidade },
          ],
        };
      }
  ...
  async function finalizarVenda() {
    ...
    try {
      const { data, error } = await vendaService.finalizarVenda(payload) 

      if (error) {
        setErroFinalizar(error)
        setDadosVenda((prevDadosVenda) => ({ ...prevDadosVenda, pin: '' }))
        return
      }
    ...
    <div className="p-6">
            ...
            <div className="space-y-3">
              <div className="rounded-2xl bg-zinc-800 border border-zinc-700 p-4">
                <p className="text-white font-semibold">
                  {dadosVenda.aluno.nome}
                </p>
                <p className="text-zinc-400 text-sm">
                  RM: {dadosVenda.aluno.rm}
                </p>
              </div>
      ...
      {etapa === "confirmar-pin" && (
        <div className="max-w-2xl space-y-4">
          ...
          {erroFinalizar && (
            <p className="text-red-500 text-sm">{erroFinalizar}</p>
          )}
